    ...
    crate::storage::StorageModule + crate::private::PrivateModule + crate::owner::OwnerModule
{
    ...
    #[endpoint(resetNft)]
    #[only_owner]
    fn reset_nft(&self, user: ManagedAddress) {
        self.require_not_paused();
        ...
        let mut nft_attr = self.blockchain().get_token_attributes::<NftAttributes>(
            &emorya_nft_payment.token_identifier,
            emorya_nft_payment.token_nonce,
        );
        nft_attr.level -= 1;
        ...
        let mut encoded_attributes = ManagedBuffer::new();
        nft_attr.top_encode(&mut encoded_attributes).unwrap();
